...
To write every expression from linear algebra as a single Einsum expression respectively was already possible before, with (TODO: quote Julien).
% There, (beschreib, was Julien gemacht hat).
With these theorems, we can derive the Einsum expression to a linear algebra expression in a different way.
...
But we still do not have a way of compressing nested Einsum expressions where a duplication is removed and another is introduced simultaniously.
