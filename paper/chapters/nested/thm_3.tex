...
\begin{proof}
    ...
    % $$\forall (\bm{f}, \bm{b}) \in \mathcal{F} \times \mathcal{B}: \bigodot\limits_{i = 1}^{m} T^{(i)}_{(\bm{f}, \bm{b}): \bm{s_i}} \odot U_{(\bm{f}, \bm{b}): \bm{\hat{s}_u}} = \begin{cases}
    ...
    %         U_{(\bm{f}, \bm{b}): \bm{s_u}} & \text{if } (\bm{f}, \bm{b}):\hat{s}_{uj} = (\bm{f}, \bm{b}):\hat{s}_{uj'} \text{ for } j,j' \in [n_u] \text{ with } s_{uj} = s_{uj'} \\
    %         \0                             & \text{else}
    %     \end{cases}$$
    % for all $(\bm{f}, \bm{b}) \in \mathcal{F} \times \mathcal{B}$ by renaming the symbols .
    % Therefore,
    ...
    % because the resulting tensor is only non-zero on multi-indices that respect the symbol duplication.
    % That is, multi-indices, that hold different indices where $\bm{s_u}$ has a duplicate symbol, are multiplied by $\0$.
    % This can be formally expressed in terms of multi-index projections:
    % $$(\bm{f}, \bm{b}): \bm{s_i} = (\bm{f}, \bm{b}): \bm{\hat{s}_i}$$
    % if
    % $$(\bm{f}, \bm{b}):\hat{s}_{uj} = (\bm{f}, \bm{b}):\hat{s}_{uj'}$$
    % for $j,j' \in [n_u]$ with $s_{uj} = s_{uj'}$.
    % In our example, this is equivalent to the following observation:
    % If $(\bm{f}, \bm{b}):j$
