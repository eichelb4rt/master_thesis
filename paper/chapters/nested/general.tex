...
$$(a,b,c,d,e,abbcde \rightarrow bc, v^{(1)}, v^{(2)}, v^{(3)}, v^{(4)}, v^{(5)}, (
    a,b,c,d \rightarrow aabccd, v^{(6)}, v^{(7)}, v^{(8)}, v^{(9)}
    )),$$
because the scope of each symbol does not reach into nested expressions,
and therefore the $a$, $b$, $c$, and $d$ used in the outer expression are treated as different symbols than the $a$, $b$, $c$, and $d$ used in the inner expression.
